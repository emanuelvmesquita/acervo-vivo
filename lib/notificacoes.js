...
// Envia uma notificação (WhatsApp, com fallback para e-mail) para um perfil
// específico e registra o resultado em notificacoes_log. Usada pelos fluxos
// de confirmação/recusa de solicitação de empréstimo e pelo cron diário.
export async function notificarUsuario(usuarioId, mensagem, tipo) {
  ...
  try {
    if (perfil?.telefone) {
      ...
      canal = "sem_canal";
      erroMsg = "Usuário sem telefone ou e-mail cadastrado.";
    }
  } catch (err) {
  ...
  await supabase.from("notificacoes_log").insert({
    tipo,
    destinatario: perfil?.nome ?? usuarioId,
    texto: mensagem,
    canal,
  ...
  return { statusEntrega, canal, erroMsg };
}
...
export async function processarNotificacoes() {
  ...
  const { data: emprestimos, error: empErr } = await supabase
    .from("emprestimos")
    .select("id, locatario, usuario_id, data_devolucao, status, exemplares(titulos(titulo))")
  ...
  for (const emp of emprestimos) {
    ...
    // Prioriza o usuário vinculado (emprestimos.usuario_id); empréstimos
    // antigos sem vínculo caem no fallback por nome (primeiro nome do locatário).
    let perfil = null;
    if (emp.usuario_id) {
    ...
    const tituloLivro = emp.exemplares?.titulos?.titulo ?? "livro";
    const nomeDestinatario = perfil?.nome ?? emp.locatario;

    const mensagem = atrasado
      ? `Olá, ${nomeDestinatario}! O livro "${tituloLivro}" do Acervo Vivo está ${Math.abs(dias)} dia(s) em atraso. Por favor, devolva o quanto antes. Obrigado!`
      : `Olá, ${nomeDestinatario}! Lembrete: o livro "${tituloLivro}" precisa ser devolvido em ${dias} dia(s) (prazo: ${fmtData(emp.data_devolucao)}). Acervo Vivo.`;
    ...
    try {
      if (perfil?.telefone) {
        ...
        statusEntrega = "pendente";
        canal = "sem_canal";
        erros.push({ locatario: nomeDestinatario, erro: "Sem telefone ou e-mail cadastrado." });
      }
    } catch (err) {
      statusEntrega = "falhou";
      erros.push({ locatario: nomeDestinatario, erro: err.message });
